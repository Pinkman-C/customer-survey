...
**Le plus simple pour la validation** : envoyer un seul lien, la page d'accueil —

```
https://survey.auctelia.be/
```

C'est un **espace de validation interne** (noindex, jamais vu par les répondants qui reçoivent des liens directs Mailchimp) : il explique le but des 4 enquêtes et la répartition des envois par segment CRM, et ses boutons ouvrent chaque formulaire **directement en mode test**, plus un accès au dashboard. Sinon, URLs directes par formulaire :
